...
##국내에서 개봉한 2150개의 영화의 매출액, 매출액점유율, 관객수, 스크린수, 상영횟수 분석
 - 데이터 컬럼 설명
 - 순위 : 매출액을 기준으로 내림차순 정렬
 - 영화명 
 - 매출액 : 영화로 벌어들이 총 매출
 - 매출액 점유율 : 모든 영화의 매출액에서 차지하는 비율
 - 관객 수 
 - 스크린 수 : 극장에서 영화가 점유하는 스크린 수
 - 상영 횟수
 - 대표 국적 : 영화를 제작한 주요 국가
 - 국적 : 영화를 보조 제작한 국가
 - 배급사 : 영화를 유통한 회사
...
#2. 해외 박스오피스의 영화 예산 통계와 평점 사이트 분석
 ...
 - Movie : 영화제목
 - Budget : 예산
 - DomesticBoxOffice : 국내 흥행수치
 - Net Profit : 총 매출액
 - IMDB : IMDB 평점. 10점 만점
 - Metacritic : Metacritic 평점. 100점 만점
 - RottenTomatoes : RottenTomatoes 평점. 100점 만점
 - RogerEbert : RogerEbert 평점. 5점 만점
```{r}
...
##IPTV Data 컬럼명
  - 순위 : 시청횟수가 많은 순서대로 내림차순.
  - FIMS 코드 : IPTV 시스템 내에서 발급하는 영화 고유 식별번호
  - 영화명 : 영화제목
  - 제작연도
  - 제작국가
  - 극장 개봉일
  ...
  - 배우
  - 등급 : 연령별 시청 가능 등급
  - X2017년 총 이용건수 : 해당 영화 2017년 IPTV 총 시청횟수
  - X2018.2월 총 이용건수 : 2018년 2월의 해당 영화 총 시청횟수
  - X2018년 총 이용건수 : 2018년 해당 영화 총 이용횟수. 이 데이터는
  2018년 2월 데이터로서 총 이용건수는 1월~2월까지 합한 시청횟수이다.
```{r}
...
박스오피스 흥행 요인 : 박스오피스에서 영화의 흥행은 자본력과 배급사가 흥행의 가장 큰 영향을 미치는 것으로 나타났다. 자본력이 많은 배급사들은 여러 상영관을 확보하여 상영 가능한 스크린 수를 배정 받을 수 있다. 스크린 수가 많을 수록 많은 관객들에게 노출이 되고 이는 곧 매출액으로 이어진다. 영화는 주변 사람들의 입소문으로 평가가 된다. 즉 많은 고객들이 영화에 노출이 되어야 마케팅 효과를 얻을 수 있어서 그만큼 더 흥행에 성공할 수 있다. 

IPTV와 박스오피스의 상관관계 : IPTV에서 영화는 영화관에서 영화가 내려갔을 때 입니다. IPTV를 이용하는 사람들은 영화관에서 흥행 했던 상영작을 위주로 시청한다. 그렇기 때문에 시청자들이 접하는 영화의 다양성은 좁으며 주로 미국과 한국 영화에 편중되어 있다. 그 중에서도 박스오피스에서 10위권 안에 든 영화들을 주로 시청한다. 박스오피스에서 흥행이 IPTV에서의 흥행으로까지 이어지는 것을 확인할수 있다.
